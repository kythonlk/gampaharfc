...
export default function MedicalClearanceForm() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-200 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto bg-gray-900 rounded-xl shadow-2xl border border-white/10 overflow-hidden">

        {/* HEADER */}
        <div className="bg-gray-950 p-8 border-b border-gray-700 text-center">
          <p className="text-white font-semibold mt-2 uppercase text-2xl tracking-wide">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-12">
          ...
          <section>
            ...
            {formData.hasInsurance === "yes" ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-green-900/10 p-6 rounded-lg border border-green-500/20">
                <InputGroup label="Insurance Provider" name="insuranceProvider" value={formData.insuranceProvider} onChange={handleInputChange} />
                <InputGroup label="Policy Number" name="policyNumber" value={formData.policyNumber} onChange={handleInputChange} />
                <InputGroup label="Policy Expiry Date" name="policyExpiry" type="date" value={formData.policyExpiry} onChange={handleInputChange} />
              ...
              <div className="bg-red-900/20 p-6 rounded-lg border border-red-500/30">
                <div className="flex gap-3 mb-4">
                  ...
                  <p className="text-sm text-gray-300 italic">
                    "I acknowledge that I do not currently possess medical insurance. I understand and accept that by participating in any training sessions, matches, events, or activities organized by Gampaha RFC, I do so entirely at my own risk. I take full responsibility for any injuries, medical expenses, or health-related issues that may occur, and I release Gampaha RFC, its officials, coaches, and representatives from any liability arising from my participation."
                  </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section>
              ...
              {Object.keys(formData.history).map((key) => (
                <label key={key} className="flex items-center gap-3 p-3 bg-white/5 rounded border border-white/10 cursor-pointer hover:bg-white/10 transition">
                  <input
                    ...
                    className="w-5 h-5 rounded border-gray-500 accent-white"
                  />
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-white mb-2">Details of Major Injuries & Surgeries</label>
                <textarea
                  name="injuryDetails"
                  value={formData.injuryDetails}
                  onChange={handleInputChange}
                  rows={3}
                  className="w-full bg-gray-950 border border-white/20 rounded p-3 text-white focus:outline-none focus:border-white transition"
                  placeholder="List significant injuries, dates, treatment, and current status..."
                ></textarea>
              </div>

              <div>
                <label className="block text-sm font-medium text-white mb-2">Current Medication</label>
                <textarea
                  name="medication"
          ...
          {/* 6. OFFICIAL USE ONLY */}
          <div className="mt-12 pt-6 border-t-4 border-white/50 bg-[#000000]/30 p-6 rounded">
          ...
          <div className="flex justify-end pt-6">
            <button
              type="submit"
              className="bg-white hover:bg-gray-200 text-black font-bold py-4 px-12 rounded shadow-lg transform hover:scale-105 transition-all uppercase tracking-wider"
            >
              Submit Declaration
            </button>
          </div>
...
const InputGroup = ({
  ...
  <div className="w-full">
    ...
    <input
      ...
      className="w-full bg-gray-950 border border-white/20 rounded px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-white focus:ring-1 focus:ring-white transition"
    />
  </div>
);
